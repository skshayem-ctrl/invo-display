...
static void fota_ui_cb(fota_state_t state, int pct, const char *msg)
{
    ...
    /* progress bar */
    lv_obj_set_width(s_fota_bar_ind,
        (lv_coord_t)((float)lv_obj_get_width(s_fota_bar) * pct / 100.0f));

    bool show_close = (state == FOTA_DONE       ||
                       state == FOTA_UP_TO_DATE  ||
                       state == FOTA_NO_WIFI     ||
                       state == FOTA_ERROR);
    if (show_close)
...
/* ── row builder ──────────────────────────────────────────────────*/
static lv_obj_t *settings_row(lv_obj_t *par, const char *icon, lv_color_t icon_col,
                               const char *label, const char *value,
                               bool clickable, lv_event_cb_t cb, int yoff)
{
...
lv_obj_t *screen_settings_create(void)
{
    ...
    mk_lbl(scr, LV_SYMBOL_SETTINGS, &lv_font_montserrat_20, C_GRAY,
           LV_ALIGN_TOP_MID, 0, 28);
    mk_lbl(scr, "Settings", &lv_font_montserrat_16, C_GRAY,
           LV_ALIGN_TOP_MID, 0, 56);

    settings_row(scr, LV_SYMBOL_WIFI,   C_BLUE,  "WiFi",       LV_SYMBOL_RIGHT,
                 true,  go_wifi_cb,       96);
    ...
    settings_row(scr, LV_SYMBOL_HOME,   C_GREEN, "System",     ver_str,
                 false, NULL,            232);

    /* ── Check Update row ──────────────────────────────────────── */
    s_update_row = settings_row(scr, LV_SYMBOL_DOWNLOAD, C_BLUE,
                                "Check Update", LV_SYMBOL_RIGHT,
                                true, check_update_cb, 300);
