...
static void fota_task(void * arg)
{
    ...
    for(int attempt = 0; attempt < 3; attempt++) {
        if(attempt > 0) {
            char rmsg[24];
            snprintf(rmsg, sizeof(rmsg), "Retry %d/3...", attempt);
    ...
    if(ota_final_err != ESP_OK) {
        if(ota_final_err == ESP_ERR_OTA_VALIDATE_FAILED)
            ...
            notify(FOTA_ERROR, 0, "Download failed (3 retries)");
        vTaskDelete(NULL);
        return;
    }
...
{
    if(s_state == FOTA_CHECKING || s_state == FOTA_DOWNLOADING || s_state == FOTA_VERIFYING) return;
    s_cb    = cb;
    s_state = FOTA_IDLE;
